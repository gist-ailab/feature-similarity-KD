...
class MobileFaceNet(Module):
    def __init__(self, fp16=False, num_features=512, blocks=(1, 4, 6, 2), scale=2, student=False, hint_bn=True):
        ...
        if self.student:
            self.hint_layer1 = HintLayer(64 * self.scale, 64, bn=hint_bn)
            self.hint_layer2 = HintLayer(64 * self.scale, 128, bn=hint_bn)
            self.hint_layer3 = HintLayer(128 * self.scale, 256, bn=hint_bn)
            self.hint_layer4 = HintLayer(128 * self.scale, 512, bn=hint_bn)
        else:
    ...
    def forward(self, x, extract_feature=False):
        ...
        with torch.cuda.amp.autocast(self.fp16):
            for stage, func in enumerate(self.layers):
                x = func(x)
                if stage in [1,3,5,7]:
                    
                    if self.student:
                        feat = getattr(self, 'hint_layer%d' %block_id)(x)
...
def get_mbf(fp16, num_features, blocks=(1, 4, 6, 2), scale=2, student=False, hint_bn=True):
    return MobileFaceNet(fp16, num_features, blocks, scale=scale, student=False, hint_bn=True)

def get_mbf_large(fp16, num_features, blocks=(2, 8, 12, 4), scale=4, student=False, hint_bn=True):
    return MobileFaceNet(fp16, num_features, blocks, scale=scale, student=student, hint_bn=hint_bn)



if __name__ == '__main__':
    model = get_mbf_large(fp16=False, num_features=512, student=True).cuda()
