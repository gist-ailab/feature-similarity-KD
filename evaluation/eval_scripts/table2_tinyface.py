...
if __name__=='__main__':
    ...
    gpus=args.gpus
    save_dir = "./result/table2/tinyface"
        ...
        for ckpt_set in ckpt_list:
            for seed in [5,4,3,2,1]:
                ...
                subprocess.call("python test_tinyface.py --gpus %s --mode %s --save_dir %s --prefix %s \
                                                         --backbone iresnet50 --pooling E --checkpoint_path %s --qualnet %s --aligned False" %(
                                    gpus, settings[0], save_dir, prefix, ckpt_path, settings[1]
                                ), shell=True)