...
do
    ...
    do
        ...
        # fi
        # RESOLUTION=0
        # python train_teacher.py --gpus 2 --seed $SEED --data_dir $DATA_DIR --save_dir checkpoint/teacher-$DATASET/iresnet50-$MARGIN-m{$M_F}-s{$S}-MP{$MP}/seed{$SEED} \
        ...
        #     M_F=0.4
        # fi
        # TEACHER=checkpoint/teacher-$DATASET/iresnet50-$MARGIN-m{$M_F}-s{$S}-MP{$MP}/seed{$SEED}/last_net.ckpt
        ...
        PARAM=20.0,4.0
        TEACHER=checkpoint/teacher-$DATASET/iresnet50-$MARGIN-m{$M_F}-s{$S}-MP{$MP}/seed{$SEED}/last_net.ckpt
        SAVE_DIR=checkpoint/student-$DATASET/$BACKBONE-$MARGIN-m{0.2}-s{$S}-MP{$MP}-logit{$LOGIT}/seed{$SEED}
        CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nnodes=1 --nproc_per_node=2 --master_port=11 train_student_multi.py --seed $SEED --data_dir $DATA_DIR --down_size $RESOLUTION \
                                                                    ...
                                                                    --mixed_precision $MP --logit_distill $LOGIT
    done
done
